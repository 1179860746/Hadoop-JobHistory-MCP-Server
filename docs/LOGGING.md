...
# 自定义日志配置
LOG_LEVEL=DEBUG \
LOG_FILE=/var/log/mcp/jobhistory.log \
LOG_MAX_SIZE=536870912 \
LOG_BACKUP_COUNT=10 \
python jobhistory_mcp.py
```
...
# 基本运行（日志输出到 stderr）
docker run -i --rm \
  -e JOBHISTORY_URL="http://hadoop:19888/ws/v1/history" \
  -e LOG_LEVEL=DEBUG \
...
services:
  jobhistory-mcp:
    image: jobhistory-mcp-server:latest
    environment:
      - LOG_LEVEL=INFO
...
```json
{
  "mcpServers": {
    "jobhistory_mcp": {
      "command": "python",
      "args": ["/path/to/jobhistory_mcp.py"],
      "env": {
        "JOBHISTORY_URL": "http://hadoop:19888/ws/v1/history",
        "LOG_LEVEL": "INFO",
        "LOG_FILE": "/var/log/mcp/jobhistory_mcp.log"
      }
    }
  }
}
